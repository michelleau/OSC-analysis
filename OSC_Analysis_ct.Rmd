...
model1 <- aov(REAC_TIME~BIRTH_EXP+AGE_GROUPS+DIS+GENDER, data = data1)
summary (model1)
TukeyHSD(model1)
pairwise.t.test(data1$AGE_GROUPS, data1$BIRTH_EXP,
                p.adjust.method = "BH")
mod <- aov (REAC_TIME~BIRTH_EXP*AGE_GROUPS, data= data1)
summary (mod)
```